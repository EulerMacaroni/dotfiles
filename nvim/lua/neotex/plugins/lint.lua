...
return {
	...
	config = function()
		...
		-- Customize chktex without discarding the built-in parser configuration
		local chktex = lint.linters.chktex
		if chktex then
			-- keep the built-in parser/args so severity parsing works
			chktex.ignore_exitcode = true -- chktex returns non-zero when warnings are present
		end
	end,
}
