...
class ImportRefactor(BaseRefactoring):
    def __init__(
        ...
        BaseRefactoring.__init__(
            self, project, resource, name="Add Import", progressCallback=None
        )
        self._name = name
        self._text = text
        self._parent = parent
    ...
    def onRefactor(self):
        ...
        if changed_source:
            ...
            self.changes.append(
                Change(self.resource.path, ChangeType.EDIT, x_diff(changeset))
            )
        else:
            raise Exception("Unknown Change")
...
class RopeRefactoring(object):
    ...
    def _add_import(self, filePath, text, name, parent, indent_size):
        ...
        resourceToRefactor = libutils.path_to_resource(project, filePath)
        refactor = ImportRefactor(project, resourceToRefactor, text, name, parent)
        refactor.refactor()
        changes = refactor.changes
        project.close()
        valueToReturn = []
